...
class MultiModalFusion(nn.Module):
    def __init__(self, in_channels, out_channels, reduction=16, kernel_size=7):
        ...
        self.norm = nn.LayerNorm((out_channels, 768))
...
class AttentionFusionBlock(nn.Module):
    def __init__(self, num_images: int = 7, embed_dim: int = 789):
        ...
        self.modal_fusion_blocks = nn.ModuleList([
            MultiModalFusion(in_channels=self.embed_dim * 2, out_channels=self.embed_dim, reduction=8, kernel_size=3) for _ in range(self.num_layers)
        ])
...
# features = torch.stack([feature_sets[j][i].view(batch_size, -1) for j in range(self.num_rgb_images)], dim=1)
# flattened_features = features.view(batch_size, -1)
# attention_weights = F.softmax(self.rgb_attention_fcs[i](flattened_features), dim=1)
