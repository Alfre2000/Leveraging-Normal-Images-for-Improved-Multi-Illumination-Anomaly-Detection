...
def performance_table(
    ...
    for class_name in classes:
        ckp_path = f'./checkpoints/rd4ad/{"fusion" if grouped else "avg"}/{"zero_shot/" if zero_shot else ""}{method}/{class_name}.pth'
        encoder, bn, decoder = load_model(architecture, ckp_path, method, grouped)
        if zero_shot:
    ...
    performance_dir = f'./performance/rd4ad/{"fusion" if grouped else "avg"}/{"zero_shot/" if zero_shot else ""}'
    ...
    **kwargs
    ...
    with torch.no_grad():
        for imgs, ground_truth, label, types in dataloader:
            ...
            anomaly_map = predict(encoder, bn, decoder, imgs, grouped=grouped, **kwargs)
...
def predict(
    ...
    for img in input_images:
        if grouped:
            ...
            features = []
            for i in img:
                features.append(encoder(i))
            # inputs = attention(features)
        else:
